...
export default function VoicePopup() {
  ...
  const startListening = () => {
    ...
    recognition.onresult = (event) => {
      ...
      operationPhrases.forEach(({ pattern, replacement }) => {
        ...
        if (before !== result)
          console.log(`Replaced phrase: ${pattern} -> ${replacement}`);
      });
    ...
    // Step 2: Replace number words
      ...
      Object.entries(allNumbers).forEach(([word, digit]) => {
        ...
        if (before !== result)
          console.log(`Replaced number: ${word} -> ${digit}`);
      });
      ...
      operations.forEach(({ pattern, replacement, name }) => {
        ...
        result = result.replace(pattern, replacement);
        if (before !== result)
          console.log(`Replaced operation: ${name} -> ${replacement.trim()}`);
      });
  ...
  const handleCalculate = async () => {
    ...
    const duration = recordingStart ? (Date.now() - recordingStart) / 1000 : 0;
